...
def get_gross_load_for_single_load_cid_sites(data, load_list, pv_list):
    """Filters input df ('data') for site_ids with no more than one c_id which records load. Then adds the PV back in
    for each site and returns a df containing a new con_type 'gross_load' with c_id = site_id *10 + 1"""
    #------------------------ First, get a list of the requisite site_ids
    c_id_and_site_id_df = pd.DataFrame({'c_id': data.groupby('c_id')['c_id'].first(),
                                        'site_id': data.groupby('c_id')['site_id'].first(),
                                        'con_type': data.groupby('c_id')['con_type'].first()})
    # Get a list of sites with a single load c_id
    sites_with_single_load_cid_list = count_num_type_X_cids_meets_criteria_Y(c_id_and_site_id_df, load_list, 1)
    ...
    for site_id in sites_with_single_load_cid_list:
        ...
        data_site_id_pv = data_site_id_pv[data_site_id_pv['clean'] == 'cleaned']
        # Copy the index to a new column for use in groupby
        data_site_id_pv['t_stamp_copy'] = data_site_id_pv.index
        # Use groupby to calc total power/energy and average frequency/power - this deals with the cases where there
        # are mulitple PV c_ids
        df_pv_merged = pd.DataFrame({'e': data_site_id_pv.groupby('t_stamp_copy')['e'].sum(),
                                     'power_kW': data_site_id_pv.groupby('t_stamp_copy')['power_kW'].sum(),
                                     'f': data_site_id_pv.groupby('t_stamp_copy')['f'].mean(),
                                     'v': data_site_id_pv.groupby('t_stamp_copy')['v'].mean(),
                                     'old_con_type': data_site_id_pv.groupby('t_stamp_copy')[
                                         'con_type'].sum()}).reset_index()
        ...
        df_pv_merged = df_pv_merged.rename(index=str, columns={'t_stamp_copy': 'ts'})
        df_pv_merged = df_pv_merged.set_index('ts')
        ...
        df_merged['e'] = df_merged['e_x'] + df_merged['e_y']
        df_merged['power_kW'] = df_merged['power_kW_x'] + df_merged['power_kW_y']
        df_merged['f'] = df_merged[['f_x', 'f_y']].mean(axis=1)
        df_merged['v'] = df_merged[['v_x', 'v_y']].mean(axis=1)
        df_merged['old_con_type'] = df_merged['old_con_type_x'] + df_merged['old_con_type_y']
        ...
        df_merged = df_merged.drop(
            ['e_x', 'e_y', 'power_kW_x', 'power_kW_y', 'f_x', 'f_y', 'v_x', 'v_y', 'old_con_type_x', 'old_con_type_y'],
            axis=1)
        # Concatenate back onto data
        data = pd.concat([data, df_merged], sort=True)
...
def count_num_type_X_cids_meets_criteria_Y(c_id_and_site_id_df, X_list, Y_criteria):
    """Input various things to get out a list of sites where type X cids meets criteria Y.
    e.g. where PV cids count = 0, so, sites with only load c_ids."""
    # Filter for X_list
    site_data = c_id_and_site_id_df[c_id_and_site_id_df['con_type'].isin(X_list)]
...
def get_gross_load_using_match_on_vrms(data_filtered_sites_list, data_without_gross_loads, ALLOWABLE_VRMS_DIFF, data):
    ...
    for site in data_filtered_sites_list:
        ...
        vrms_compare_df = pd.DataFrame({'v_sum' : data_site.groupby('c_id')['v'].sum()}).reset_index()
        counter = 1
        ...
        for current_row in range(0,len(vrms_compare_df)):
            ...
            for row in range(current_row+1, len(vrms_compare_df)):
                ...
                # Merge the 'current_cid' and 'row_cid' dfs - this is time series data!
                ...
                combined_df['diff_v'] = abs(combined_df['v_x'] - combined_df['v_y'])
                total_diffs = combined_df['diff_v'].sum() # Find the sum of these differences
                print(total_diffs)
        ...
        for counter in match_counter_list:
            ...
            data_site_match['t_stamp_copy'] = data_site_match.index
            # Use groupby to calc total power/energy and average frequency/power
            c_id_gross_load_df = pd.DataFrame({'e' : data_site_match.groupby('t_stamp_copy')['e'].sum(),
                                               'power_kW' : data_site_match.groupby('t_stamp_copy')['power_kW'].sum(),
                                               'f' : data_site_match.groupby('t_stamp_copy')['f'].mean(),
                                               'v' : data_site_match.groupby('t_stamp_copy')['v'].mean(),
                                               'old_con_type': data_site_match.groupby('t_stamp_copy')['con_type'].sum()
                                               }).reset_index()
            # Sort index (just in case) and rename t_stamp_copy to t_stamp, then return it to the index
            c_id_gross_load_df = c_id_gross_load_df.sort_index()
            c_id_gross_load_df = c_id_gross_load_df.rename(index=str, columns = {'t_stamp_copy' : 'ts'})
            c_id_gross_load_df = c_id_gross_load_df.set_index('ts')
            # Update c_id name
            c_id_gross_load_df['con_type'] = 'gross_load'
            # Create a new c_id label for gross load, using the site_id * 10 + 1 (e.g. it should go from 1562 to 15621)
            gross_c_id = (site * 10) + counter
            ...
            # Add meta data back on before trying to concat onto main df (data
            # Get the site_data without energy, power, power_kW, frequency or vrms.
            # Merge with c_id_gross_load_df, KEEPING ONLY THE INDEX FROM c_id_gross_load_df
            data_site_meta = data_site.drop(['e', 'power_kW', 'f', 'v', 'old_con_type', 'con_type', 'c_id'], axis = 1)
            # add index as col then drop dups
            data_site_meta['t_stamp_copy'] = data_site_meta.index
            data_site_meta = data_site_meta.drop_duplicates(subset=['t_stamp_copy'])
            # Merge using only the index from c_id_gross_load_df
...
def ffil_on_cids(data):
    ...
    for c_id in list_unique_cids:
        print(str(counter) + '/' + str(len(list_unique_cids)))
        ...
        # Flag missing data
        c_id_data_nans.loc[c_id_data_nans['c_id'].isnull(), 'missing_val_flag'] = 1.0
        # Alt method: 
...
def ffil_data_by_cids(data, time_stamp_name):
    ...
    # Set up outputdf
    data_output = pd.DataFrame(columns = data.columns.values.tolist())
    data_output.index.name = time_stamp_name
    ...
    for c_id in list_unique_cids:
        ...
        c_id_data_nans = index_df.merge(c_id_data, left_index=True, right_index=True, how = 'left')
        # Flag nans
        # First, get list of columns for ffill below, NOTE this must be got before adding the nan_flag col, else the
        # nan_flags will also be ffilled
...
def check_for_substantial_negative_vals_in_profile(data, con_type_list, ALLOWED_GROSS_LOAD_NEGATIVE_THRESHOLD_KW,
                                                   purpose_is_to_relabel_to_gross_YN):
    """Checks for cases where 'gross load' c_ids have values below the allowed gross load negative threshold. 
    IF 'purpose_is_to_relabel_to_gross_YN' == 'N' then Flags these c_ids as 'note_3_gross_with_large_negative_vals' = 1
    IF 'purpose_is_to_relabel_to_gross_YN' == 'Y' then Flags these c_ids as 'note_2_relabelled_gross_from_net' = 1 and
    relabels con_type as 'gross_load'"""
    # Note 3) missing load or PV... tricky. Look for negatives in c_id gross load - there shouldn't be any!
    data_gross_only = data[data['con_type'].isin(con_type_list)]
    ...
    for c_id in c_ids_in_data_gross_only:
        ...
        negative_power_sum = abs(c_id_data.loc[c_id_data['power_kW'] < ALLOWED_GROSS_LOAD_NEGATIVE_THRESHOLD_KW,
                                               'power_kW'].sum()) # Maybe use -0.5kW instead of 0.0?

        if purpose_is_to_relabel_to_gross_YN == 'Y':
            if negative_power_sum == 0:
                data.loc[data['c_id'] == c_id, 'con_type'] = 'gross_load'
                data.loc[data['c_id'] == c_id, 'note_2_relabelled_gross_from_net'] = 1.0

        # If there is negative stuff (i.e. negative_power_sum != 0) then flag it!
        if purpose_is_to_relabel_to_gross_YN == 'N':
            if negative_power_sum > 0.0:
                data.loc[data['c_id'] == c_id, 'note_3_gross_with_large_negative_vals'] = 1.0
    return data
...
def filter_out_sites_with_low_output(ts_df, con_type_list, APPROX_ZERO_THRESHOLD):
    """Checks for all c_ids with con_type in the passed con_type_list that the max power is at least equal
    to the APPROX_ZERO_THRESHOLD. Filters out those c_ids which are not"""
    ts_df_gross_temp = ts_df[ts_df['con_type'].isin(con_type_list)]
    gross_load_max_vals = pd.DataFrame(ts_df_gross_temp.groupby('c_id')['power_kW'].max()).reset_index()
    small_gross_loads = gross_load_max_vals[gross_load_max_vals['power_kW'] < APPROX_ZERO_THRESHOLD]['c_id'].tolist()
    return_df = ts_df[np.logical_not(ts_df['c_id'].isin(small_gross_loads))]
    return return_df

def filter_out_sites_with_missing_data_during_event(ts_df, EVENT_TIME, TIME_INTERVALS, TIME_STEPS):
    """Checks for all cleaned c_ds whether there is data at the event time, and for TIME_STEPS number of intervals
     immediately after the event. Filters out those which are not."""
    temp_df = ts_df[ts_df['clean'] == 'cleaned']
    # Filter for event window. First, get the end time.
    event_end_time = EVENT_TIME + pd.Timedelta(seconds=TIME_INTERVALS) * TIME_STEPS
    # Get time only (i.e. not date) and convert to string
    event_end_time = str(event_end_time)[7:]
    # Then apply filter using this string
    temp_df = temp_df.between_time(EVENT_TIME, event_end_time)
    # Count the number of data points by c_id and filter out any c_ids with a count < the num of TIME_STEPS
    c_id_data_count = pd.DataFrame(temp_df.groupby('c_id')['power_kW'].count()).reset_index()
    c_id_with_missing_data = c_id_data_count[c_id_data_count['power_kW'] < (TIME_STEPS + 1)]['c_id'].tolist()
    ts_df = ts_df[np.logical_not(ts_df['c_id'].isin(c_id_with_missing_data))]
    return ts_df

def calc_num_sites_and_cids(ts_df_final, PV_LIST):
    """finds the number of sites and c_ids in the df"""
    temp_df = ts_df_final[ts_df_final['con_type'].isin(PV_LIST)]
    temp_df = temp_df[temp_df['clean']=='cleaned']
    num_clean_pv_sites = len(temp_df['site_id'].drop_duplicates())
    num_clean_pv_c_ids = len(temp_df['c_id'].drop_duplicates())
    return  num_clean_pv_sites, num_clean_pv_c_ids
